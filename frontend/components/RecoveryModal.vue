...
const handleAction = async (target: string, action: string) => {
  ...
  try {
    console.log(
      `Запрос к Go: RecoverComponent(key: '${target}', force: ${isForceReinstall})`,
    );
...
<template>
  <div
    class="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[100005]"
    @click.self="emit('close')"
  >
    <div
      class="bg-gray-900 border border-gray-700 rounded-2xl p-8 w-[600px] flex flex-col gap-6 text-primary shadow-2xl"
    >
      <h2 class="text-3xl font-bold text-center tracking-wider">
        Восстановление
      </h2>
      ...
      <div class="flex flex-col gap-4">
        <!-- Proton/Wine -->
        <div
          class="flex flex-row justify-between items-center bg-white/5 p-4 rounded-xl border border-white/10"
        >
